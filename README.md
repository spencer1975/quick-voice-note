...
Long voice memos for Obsidian. One tap starts a recording. Stopping saves the
...
- **Memo style:** list item, voice callout (a tinted card with a mic icon,
  in the colour and wash strength you pick), quote block, heading per memo, plain
...
Recordings become readable text via any transcription API that speaks the OpenAI format.
Bring your own key (OpenAI, Groq, or a local Whisper server) under
**Settings → Advanced → Transcription service**.

Prefer zero setup? A **Cloud key** (A$9 a month, 3 days free) gives you
